...
def get_selected_column_idx(all_features, selected_lmks, axes):
    ...
    selected_column_idx = [
        i for i in range(len(all_features)) if all_features[i] in selected_features
    ]
...
def nan_mean(x, axis=None):
    ...
    sum = tf.math.reduce_sum(
        tf.where(valid_mask, x, tf.zeros_like(x)), axis=axis, keepdims=True
    )
    weight = tf.math.reduce_sum(
        tf.cast(valid_mask, tf.float32), axis=axis, keepdims=True
    )
    return sum / weight
...
def preproc_norm(inputs, **kwargs):
    ...
    coord = normalize(coord, refs, axes)
    coord = pad(coord, max_len)
    output = tf.where(tf.math.is_nan(coord), 0.0, coord)
...
def tokenize_and_encode(text, chr2idx, label_len):
    tokens = [START_TOKEN] + list(text) + [END_TOKEN] 
    encoded_tokens = [chr2idx[char] for char in tokens]  
    if len(encoded_tokens) < label_len:
        encoded_tokens += [0] * (label_len - len(encoded_tokens))
    else:
        encoded_tokens = encoded_tokens[:label_len]
    return encoded_tokens


def create_tf_example(df, df_label, chr2idx, **kwargs):
    sequence_id = df.index[0]
    label = df_label[df_label["sequence_id"] == sequence_id]["phrase"].values[0]
    label_len = kwargs.get("label_len", 30)
    encoded_label = tokenize_and_encode(label, chr2idx, label_len)
    ...
    flat_data = preproc_data.numpy().flatten()

    feature_dict = {
        # "sequence_id": _int_feature([sequence_id]),
        # "seq_len": _int_feature([seq_len]),
        # "lm_dim": _int_feature([lm_dim]),
        "source": _float_feature(flat_data),
        "target": _int_feature(encoded_label),
    }
...
def write_to_tfrecord(prquet_file, df_label, chr2idx, writer, **kwargs):
    df = pd.read_parquet(prquet_file)
    sequence_ids = df.index.unique()
    for sequence_id in sequence_ids:
        sub_df = df.loc[[sequence_id]]
        example = create_tf_example(sub_df, df_label, chr2idx, **kwargs)
...
def main():
    ...
    chr2idx_path = path.join(data_dir, "character_to_prediction_index.json")
    chr2idx = json.load(open(chr2idx_path))
    chr2idx[START_TOKEN] = len(chr2idx)
    chr2idx[END_TOKEN] = len(chr2idx)
    ...
    max_len = 300
    ref_lmrk_id = 489  # x_pose_0 (near the nose)
    preproc_method = "norm"
    label_len = 30
    ...
    with open(preproc_args_path, "w") as f:
        json.dump(preproc_args, f)

    with tf.io.TFRecordWriter(tfrecords_path) as writer:
        for parquet_path in tqdm.tqdm(parquet_paths):
            write_to_tfrecord(parquet_path, df_label, chr2idx, writer, **preproc_args)
...
if __name__ == "__main__":
    ...
    # cols = [col for col in df.columns if re.search(r"(x_|y_|z_)", col)]
    # sequence_ids = np.unique(df.index)
    # sequence_id = sequence_ids[0]

    # df = df.loc[[sequence_id]]
    # data = df[cols].values
    # preproc_data = preproc_hub(data, **preproc_args)
